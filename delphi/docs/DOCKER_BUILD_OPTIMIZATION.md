...
- ✅ Use `make docker-build` for development (fast rebuilds)
- ✅ Regenerate `requirements.lock` when updating `pyproject.toml`
- ✅ Commit `requirements.lock` to version control
- ✅ Review dependency changes in PRs
- ✅ Use `docker-build-no-cache` to verify clean builds work

### DON'T

- ❌ Edit `requirements.lock` manually (regenerate instead)
- ❌ Remove `requirements.lock` from the repository
- ❌ Skip regenerating lock file after dependency updates
- ❌ Use `--no-cache-dir` in Dockerfile builder stage (slows rebuilds)
- ❌ Copy source code before installing dependencies (breaks caching)
...
Use `make docker-build` for daily development and enjoy fast iteration cycles! 🚀
